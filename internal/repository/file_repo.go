...
func (r *FileRepo) List(offset, limit int) ([]model.File, int64, error) {
	var files []model.File
	var total int64
	r.db.Model(&model.File{}).Count(&total)
	if err := r.db.Offset(offset).Limit(limit).Find(&files).Error; err != nil {
		return nil, 0, err
	}
